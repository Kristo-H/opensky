...
raw_data = pd.read_csv("opensky_states_snapshot.csv")
...
# filters need to be made -> removing anomalies in G1


# After removing identified anomalies and filtered dataset in G1
# loading filtered dataset
filtered_data = pd.read_csv("filtered_final_data.csv")
...
# plot ("geo_altitude", "velocity") pair in original units
...
for x, y in feature_combinations:
    ...
    #plot geo_altitude vs velocity cluster dbscan, in original units
    plt.figure(figsize=(10,6))
...
# dbscan found 3 more points as outliers, in total 8. 
# k-means found 5 in total.

# Graph 2. baro_altitude vs velocity cluster on standardized data:
# dbscan found 4 more points as outliers, in total 9.
# k-means found 5 in total.

# Graph 3. geo_altitude vs baro_altitude cluster on standardized data:
# dbscan found 3 points in total as outliers.
# k-means however found 0 in total.

# Explored multiple clustering methods (K-means, DBSCAN).
# Found out that K-means always creates clusters and it won't flag some outliers, 
