...
data = pd.read_csv("data\opensky_states_snapshot.csv")
print(data.head())
...
for column in categorical_columns:
    ...
    plt.show()
"""
# conclusion -> no patterns to explore in icao24, callsign, squawk (unique codes or names)
# columns that can be explored: origin_country, longitude, 
# latitude, baro_altitude, on_ground, velocity, vertical_rate, geo_altitude, spi
...
# Plot for patterns in: origin_country, on_ground, spi
#plot top 10 origin_country
...
plt.xticks(rotation=45)
plt.show()
# top 10 countries for flights are: United States, United Kingdom, Germany, Ireland, 
# Turkey, China, Spain, France, Malta, Japan
...
# find where spi(special purpose indicator flag) is true
print()
print("spi true:")
print(data[data["spi"] == True])

# find where on_ground is true to check velocity, geo_altitude, baro_altitude
...
print(on_ground_data_sorted_rows) # more than 20 rows are duplicates and anomalies


# check count, mean, std, min, max, quartiles for these numerical columns
print()
...
# 50% of baro_altitudes are <= 9106m, 50% of geo_altitudes <= 9601m, 50% of velocities <= 211m/s


# check rows for impossible values (shows where geo_altitude or baro_altitude or velocity is negative)
...
# conclusion -> these values are within possible values, no need to remove
...
# check top 1 geo_altitude 21046.44m for baro_altitude and velocity:
print()
print("top 1 geo_altitude:")
print(data.loc[data["geo_altitude"].idxmax(), 
               ["geo_altitude", "baro_altitude", "velocity"]])
# conclusion -> top 1 geo_altitude 21046.44m with baro_altitude 20086.32m and velocity 2.06 m/s is possible for an aircraft (surveillance balloon)
...
#conclusion: geo_altitudes are in normal ranges, no filtering needed
...
# conclusion -> top 1 baro_altitude 21061.68m with geo_altitude 121.92m and velocity 51.51 m/s is not possible for an aircraft
...
#conclusion: values with row indexes 6125 and 1003 have high difference between baro_altitude and geo_altitude -> apply corrections.
...
print()
print("altitude correction by percentage")
...
print()
print("top 20 rows with largest altitude difference percentage")
...
print()
print("altitude difference with other columns")
print(data[["geo_altitude", "baro_altitude", "altitude_diff"]].sort_values(by="altitude_diff", ascending=False))
# conclusion: large values for altitude_diff(9601.2m and -20939m). geo_altitude and baro_altitude need to be in correlation -> remove large values

# export to CSV to view all altitude_diff
data[["geo_altitude", "baro_altitude", "altitude_diff"]].sort_values(by="altitude_diff", ascending=False).to_csv("data/altitude_diff_full.csv", index=False)
...
data_filtered[["geo_altitude", "baro_altitude", "altitude_diff"]].sort_values(by="altitude_diff", ascending=False).to_csv("data/altitude_diff_full_filtered.csv", index=False)


# check top 20 velocities
print()
print("top 20 velocities: ")
print(data["velocity"].sort_values(ascending=False).head(20))
print()
print("top 20 velocity values with columns icao24, callsign, geo_altitude and baro_altitude")
print(data_filtered.sort_values("velocity", ascending=False).head(20)[["icao24", "callsign", "velocity", "baro_altitude", "geo_altitude"]])


# filter out planes that are above 340m/s per second (1 Mach is the speed of sound, these aircraft by icao24(aircraft identifier) don't reach that speed)
...
data_filtered.sort_values("velocity", ascending=False)[["callsign","velocity","baro_altitude","geo_altitude"]].to_csv("data/velocity.csv", index=False)

# remove duplicates found before: baro_altitude=9105.9, geo_altitude=9601.2, velocity < 25 . (parked aircraft and aircraft with slow velocity can not be at 9000m altitude)
...
data_filtered.sort_values("velocity", ascending=False)[["callsign","velocity","baro_altitude","geo_altitude"]].to_csv("data/velocity_filtered.csv", index=False)

# Check lowest velocities after filtering
print()
print("lowest velocities after filtering")
print(data_filtered.sort_values("velocity")[["callsign","velocity","baro_altitude","geo_altitude"]].head(20))
...
data_filtered.sort_values("vertical_rate", ascending=False)[["icao24", "callsign","velocity","baro_altitude","geo_altitude", "vertical_rate"]].to_csv("data/vertical_rate.csv", index=False)
...
data_filtered.to_csv("data/filtered_final_data.csv", index=False)
...
# visualizing longitude, latitude on the World map using template from GeoPandas
world_map = gpd.read_file("src/worldmap/ne_110m_admin_0_countries.shp")
...
# most flights are from United States, United Kingdom, Europe
# there are also flights from Brazil, South Africa, Australia, Indonesia, ...


# plot airplane positions on world map, colored with geo_altitude
fig, ax = plt.subplots(figsize=(16, 8))
world_map.plot(ax=ax, color="lightgray", edgecolor="black")
...
# Germany and United Kingdom have many flights between altitudes 0-7500m


# plot airplane positions on world map, colored with velocity
fig, ax = plt.subplots(figsize=(16, 8))
world_map.plot(ax=ax, color="lightgray", edgecolor="black")
...
plt.show()
# shows that flights in:
# for example East Coast of the United States have velocity concentrated between 200-250 m/s
# Japan has velocity concentrated between 250-280 m/s
# United Kingdom and Germany have flights with velocities between 0-100 m/s
